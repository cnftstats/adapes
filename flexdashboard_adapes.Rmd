...
<ul id="sidebar_ul">
<li id="currentProject"><a href="https://cnftstats.github.io/adapes">ApeSociety</a></li>
...
<li><a href="https://cnftstats.github.io/claynation">ClayNation</a></li>
...
X[price %between% c(0, 1999),      price_range := trunc(price/100)*100]
X[price %between% c(2000, 4999),   price_range := trunc(price/500)*500]
X[price %between% c(5000, 9999),   price_range := trunc(price/1000)*1000]
X[price %between% c(10000, 29000), price_range := trunc(price/10000)*10000]
...
X[price_range %between% c(0, 1999),    x_label := paste0(price_range, "-", price_range + (100-1))]
...
X[price %between% c(10000, 29000), price_range := trunc(price/10000)*10000]
X[price >= 30000,                  price_range := 30000]
X <- X[complete.cases(price_range), .(price_range, price, market, asset_trait_class)]
X[, asset_class := gsub("class_", "", asset_trait_class)]
X[, asset_class := factor(asset_class,
                          levels = rev(c("Kings", "Nobles", "Royal Advisors", "Military Officers",
                                         "Merchants", "Explorers", "Artists", "Craftsmen")))]

CAP <- X[, .(floor = min(price), median = floor(median(price)),
             listings = .N), asset_class][order(asset_class)]
CAP_all <- X[, .(asset_class = "All", floor = min(price),
                 median = floor(median(price)), listings = .N)]
CAP <- rbind(CAP_all, CAP)
.pattern <- "%s: Floor = %d | Median = %d | Listings = %d"
.caption <- paste0(CAP[, sprintf(.pattern, asset_class, floor, median, listings)], collapse = "\n")
X <- X[, .N, .(price_range, market, asset_class)][order(price_range)]
X[price_range %between% c(0, 1999),    x_label := paste0(price_range, "-", price_range + (100-1))]
X[price_range %between% c(2000, 4999), x_label := paste0(price_range, "-", price_range + (500-1))]
X[price_range %between% c(5000, 9999), x_label := paste0(price_range, "-", price_range + (1000-1))]
X[price_range %between% c(10000, 29999), x_label := paste0(price_range, "-", price_range + (10000-1))]
...
ggplot(X, aes(x_label, N, fill = market)) +
  facet_wrap(.~asset_class, nrow = 3, ncol = 3) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = stat(y), group = x_label),
            stat = 'summary', fun = sum, vjust = -.5, hjust = +.55,
            color = "steelblue4", size = 3) +
...
### Sales x class (last 24h)
```{r fig.height=8, fig.width=10}
X <- copy(DTS)
X <- X[trunc(sold_at_days) == 0]
...
CAP <- X[, .(floor = min(price), roof = max(price), sales = .N,
             volume = sprintf("%.0fk", sum(price)/1000)), asset_class][order(asset_class)]
CAP_all <- X[, .(floor = min(price), roof = max(price), sales = .N,
                 volume = sprintf("%.0fk", sum(price)/1000), asset_class = "All")]
CAP <- rbind(CAP_all, CAP)
.pattern <- "%s: Sale floor = %s | Sale roof = %s | Sales = %s | Vol. = %s"
.caption <- paste0(CAP[, sprintf(.pattern, asset_class, floor, roof, sales, volume)], collapse = "\n")
...
### Sweep that floor: Lowest listings by class
```{r}
X <- DT[, .SD[price == min(price)], asset_trait_class]
X[, asset_class := gsub("class_", "", asset_trait_class)]
X[, asset_class := factor(asset_class,
                          levels = c("Kings", "Nobles", "Royal Advisors", "Military Officers",
                                     "Merchants", "Explorers", "Artists", "Craftsmen"))]
...
X <- X[, .(asset_class, img, price, asset_rank, market)][order(asset_class)]
...
datatable(X, class = 'compact stripe hover',
          ...
          colnames = c("Class", "Asset", "Price", "Rank", "Market"),
          rownames = FALSE,
          escape = FALSE) %>%
  formatStyle(columns = 1:ncol(X), fontSize = '90%')
```

### Sweep that floor: Overall lowest listings
```{r}
...
X[, img := sprintf("https://raw.githubusercontent.com/cnftstats/images/main/adapes/%s_large.png", asset_name)]
...
gg <- ggplot(X, aes(asset_rank, price, color = asset_class, customdata = custom_data,
                    text = paste0(asset,
...
ggplotly(gg, tooltip = "text") %>% 
  layout(legend = list(x = 0.01, y = .99, bgcolor = 'rgba(255,255,255,0.6)'),
...
> Only listings below `r ifelse(max_y > 1000, sprintf("%.0fk", max_y/1000), max_y)` ada are displayed. Try clicking on the legend or datapoints.
...
gg <- ggplot(X, aes(asset_rank, price, color = asset_class, customdata = custom_data,
  ...
  geom_point(alpha = .6) +
  scale_x_reverse(breaks = scale_x, expand = c(0.03, 0), limits = c(max_x, min_x)) +
  scale_y_continuous(n.breaks = 15, limits = c(min_y
                                               , max_y)) +
...
> Only listings below `r ifelse(max_y > 1000, sprintf("%.0fk", max_y/1000), max_y)` ada are displayed.

## Column 2 {data-width=300}
### Floor listings by rank range
```{r}
X <- DT[, .SD[price == min(price)], rank_range]

X[, img := sprintf('<img src="%s%s.png" class="img_html">', img_link, gsub("adape", "", asset_name))]
X[, img := sprintf('<a href="%s" target="_blank">%s</br>#%s</a>', link, img, gsub("adape", "", asset_name))]

X <- X[, .(rank_range, img, price, asset_rank, market)][order(rank_range)]
X <- X[complete.cases(X)]
...
datatable(X, class = 'compact stripe hover',
          options = list(dom = "t",
                         ...
                         info = FALSE,
                         columnDefs = list(list(className = 'dt-right', targets = "_all"))),
          colnames = c("Rank range", "Asset", "Price", "Rank", "Market"),
          rownames = FALSE,
          escape = FALSE) %>%
  formatStyle(columns = 1:ncol(X), fontSize = '90%')
```
...
<!-- ### Valuate your NFT -->
<!-- ```{r} -->
<!-- Y <- DTL[, .SD[price == min(price)], .(trait_category, trait)][order(trait_category, trait)] -->
<!-- Y <- Y[, .(trait_category, trait, price, asset_rank, market, asset_traits)] -->
<!-- Y[, trait_category_id := paste0(trait_category, "_", trait)] -->

<!-- .cols <- names(RAR)[names(RAR) %like% "^asset_trait_"] -->
<!-- V <- data.table(tidyr::gather(RAR, key, trait_category_id, all_of(.cols))) -->
<!-- V[, trait_category := strsplit(trait_category_id, "_")[[1]][1], 1:nrow(V)] -->
<!-- V[, trait_category := gsub("\\.", " ", trait_category)] -->
<!-- V[, trait          := strsplit(trait_category_id, "_")[[1]][2], 1:nrow(V)] -->

<!-- loj(V, Y[, .(trait_category_id, price)], "trait_category_id") -->

<!-- V <- V[, .(asset_name, asset_rank, best_trait = trait_category_id, value_best_trait = price)] -->
<!-- V <- V[, .SD[value_best_trait == max(value_best_trait)], asset_name] -->
<!-- V <- V[order(value_best_trait, asset_name)] -->
<!-- V[, best_trait_list := paste(best_trait, collapse = " - "), asset_name] -->
<!-- V <- V[, .(asset_name, asset_rank, best_trait_list, value_best_trait)] %>% unique -->
<!-- V[, value_collection_floor := min(DT$price)] -->

<!-- margin_rank <- floor(2.5/100 * max(RAR$asset_rank)) -->
<!-- V[, value_rank := { -->
<!--   .rank <- asset_rank -->
<!--   .min_rank_low <- DT[asset_rank %between% c(.rank - margin_rank, .rank), min(price)] -->
<!--   .min_rank_high <- DT[asset_rank %between% c(.rank, .rank + margin_rank), min(price)] -->

<!--   floor((.min_rank_low + .min_rank_high)/2) -->

<!--   # DT[asset_rank %between% c(.rank - margin_rank, .rank + margin_rank), round(median(price), 0)] -->
<!-- }, 1:nrow(V)] -->


<!-- V[, .(asset_name, value_collection_floor, -->
<!--       best_trait_list, value_best_trait,  -->
<!--       asset_rank, value_rank)] %>%  -->
...
<!--                          info = TRUE, -->
<!--                          columnDefs = list(list(className = 'dt-right', targets = "_all"))), -->
<!--           colnames = c("Asset", "Value</br>Collection floor", -->
<!--                        "Best trait(s)", "Value</br>Best trait(s)", -->
<!--                        "Rank", "Value</br>Rank"), -->
<!--           filter = "top", -->
<!--           rownames = FALSE, -->
<!--           escape = FALSE) %>% -->
